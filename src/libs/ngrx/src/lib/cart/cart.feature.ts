...
export const cartReducer = createReducer(
  ...
  on(addToCart, clearCart, removeCartItem, modifyQuantity, (state) => ({
    ...state,
...
export const cartFeature = createFeature({
  ...
  extraSelectors: ({ selectCart }) => ({
    selectCartItems: createSelector(selectCart, (cart: Cart | null) => {
    ...
    selectCartTotal: createSelector(selectCart, (cart: Cart | null) => {
      if (!cart || !cart.cartItems) return 0;
      const items = cart.cartItems;
      const total =
        items.length === 0
          ? 0
          : (items as CartItemDetail[]).reduce(
